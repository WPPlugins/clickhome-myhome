...
class MyHomeAdvertising{
   ...
   */
  private static $CRONJOB_HOOK='myhomeadvertising';
  ...
  public function updateDatabase(){ //myHome()->log->info('updateDatabase()');
    ...
    if(!in_array($templateHouseTypes,$templates)){
      myHome()->log->error(sprintf('Template file "%s" not available in the current theme, using default template in house types',
        $templateHouseTypes));
    ...
    if(!in_array($templateDisplays,$templates)){
      myHome()->log->error(sprintf('Template file "%s" not available in the current theme, using default template in displays',
        $templateDisplays));
    ...
    foreach(MyHomeHouseType::findAllPages() as $houseTypeId=>$page)
      if(!in_array($houseTypeId,$houseTypeIds)){
    ...
    foreach(MyHomeDisplay::findAllPages() as $displayId=>$page)
    ...
    $documentUrls=$this->findDocUrls($houseTypes,$displays);

    $newAttachmentsDocs=0;
    $unknownDocTypes=0;

    foreach($documentUrls as $documentUrl=>$type){
      ...
      $document=$api->download(['documents',$documentUrl],$authentication);

      $this->docCreateAttachment($documentUrl,$type,$mimeType,$document);

      $newAttachmentsDocs++;
    }

    return compact('newPagesHouseTypes','deletedPagesHouseTypes','newPagesDisplays','deletedPagesDisplays',
      'newAttachmentsDocs','unknownDocTypes');
  }
  ...
  private function docCreateAttachment($documentUrl,$type,$mimeType,$document){
    ...
    $url=sprintf('%s/%u.%s',$wpUploadDir['url'],$documentUrl,$type);
    $path=sprintf('%s/%u.%s',$wpUploadDir['path'],$documentUrl,$type);

    if(@file_put_contents($path,$document)===false)
      throw new MyHomeException(sprintf('Could not save the document %u to %s',$documentUrl,$path));

    $attachment=['guid'=>$url,
      'post_mime_type'=>$mimeType,
      'post_title'=>sprintf('Document %u',$documentUrl),
      'post_status'=>'inherit',
      'post_content'=>''];
  ...
  private function findDocUrls(array $houseTypes,array $displays){
    ...
    foreach($houseTypes as $houseType){
      foreach($houseType->housedocs as $doc)
        if(!isset($urlsTypes[$doc->url]))
          $urlsTypes[$doc->url]=$doc->type;

      foreach($houseType->facades as $facade)
        foreach($facade->facadedocs as $doc)
          if(!isset($urlsTypes[$doc->url]))
            $urlsTypes[$doc->url]=$doc->type;

      foreach($houseType->planoptions as $planoption)
    ...
    foreach($displays as $display)
      foreach($display->planoptions as $planoption)
        foreach($planoption->planoptiondocs as $doc)
          if(!isset($urlsTypes[$doc->url]))
            $urlsTypes[$doc->url]=$doc->type;
