...
class MyHomeFacebook{
  ...
  public function detectLogin(){
    ...
    try{
      $response=$fb->get('/me?fields=id,email,picture',$token);
    }
    catch(Exception $e){
      // If an error occured, don't try to reuse this token
      $this->forgetUserToken();
  ...
  public function testApp(){
    ...
    $endpoint=
      sprintf('/%s?fields=id,name,contact_email,supported_platforms,website_url',myHome()->options->getFacebookAppId());
