...
class MyHomeApi{
  ...
  public function authenticationHeaders($username,$password,$jobNumber){
    return ['authorization'=>'Basic '.base64_encode(sprintf('%s:%s',$username,$password)),
  ...
  public function download($command,array $authentication=null){
    //myHome()->log->info('download() ' . json_encode($command) . ', auth: ' . serialize($authentication));
   ...
   */
  private function request($command,$requestMethod,array $params,array $authentication=null,$defaultArray=false,
    $parse=true,$acceptContent='application/json'){
    //myHome()->log->info("\n-------------------------------\n");
    if(!in_array($requestMethod,['GET','POST','PUT','DELETE']))
      throw new MyHomeException('Request method not supported: '.$requestMethod);

    $url=$this->url($command);
    myHome()->log->info($requestMethod . ': ' . $url . ', params: ' . json_encode($params) . ', auth: ' . $authentication['authorization']);
    ...
    if($requestMethod==='GET'){
      ...
      $options=[CURLOPT_URL=>$url,
        CURLOPT_POST=>true,
        CURLOPT_POSTFIELDS=>json_encode($params)]; // The fields are sent in JSON format
    ...
    myHome()->log->info('options: ' . json_encode($options));
    ...
    //myHome()->log->info('responseHeadr: ' . $header);
    //myHome()->log->info('response: ' . $response);
    ...
    if($error!==null) {
      ...
      if(isset($decodedResponse->status) && isset($decodedResponse->error)) {
        ...
        myHome()->handleError(sprintf('HTTP error code %u: %s', $decodedResponse->status, $decodedResponse->error));
      }
    ...
    //myHome()->log->info('$parsedResponse: ' . $parsedResponse);


    // The parser returns null when a malformed response is received
    if($parsedResponse===null){
      $this->setLastError(sprintf(__('Wrong response format (%s %s)','myHome'),$requestMethod,$command),
        self::$ERROR_TYPE_OTHER);

      return null;
    }
    // The parser returns an empty string when an empty (but valid) response is received
    ...
    if(preg_match('|^Content-Disposition: (.+)$|m',$header,
      $contentDisposition)) // cURL does not provide any other means to get the Content-Disposition header
  ...
  private function resetCurl($acceptContent,$extraOptions=[],$extraHeaders=[],array $authentication=null){
    ...
    $options=
      [CURLOPT_FOLLOWLOCATION=>false,//true, // Follow redirections (when a "Location" HTTP header is present in the response)
        ...
        CURLOPT_HTTPHEADER=>$headers]; // Set the remaining HTTP headers
   ...
   */
  private function url($command){
    if(!is_array($command))
      $commands=[$command];
    else
      $commands=$command;

    return sprintf('%s%s/%s',trailingslashit($this->endpoint),self::$COMMAND_PREFIX,implode('/',$commands));
  }
