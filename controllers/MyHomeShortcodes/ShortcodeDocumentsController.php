...
class ShortcodeDocumentsController extends MyHomeShortcodesBaseController{
  ...
  public function doGet(array $params=[]){
    ...
    list($documentId,$inline,$thumb,$cache,$action)=$this->extractParams(['myHomeDocumentId',
      'myHomeInline',
      'myHomeThumb',
      'myHomeCache',
      'myHomeAction'],$params);

    //myHome()->log->info('docContr doGet: ' . json_encode($params));
    ...
    $varName=sprintf('document%s%u',$thumb?'Thumb':'',$documentId);
    ...
    myHome()->log->info('route: ' . $route . ', action: ' . $action . ', token: ' . json_encode($authentication));
   ...
   */
  protected function documentDownloadUrl($documentId){
    $formAttributes=$this->formAttributes;

    $formAttributes['params']['myHomeDocumentId']=$documentId;
    $formAttributes['params']['myHomeInline']=
      (int)false; // add_query_arg() ignores parameters with a boolean false value
