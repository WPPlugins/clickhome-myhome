...
class ShortcodeMaintenanceIssuesController extends ShortcodeMaintenanceBaseController{
  ...
  protected function doPostMaintenance(array $params){
    list($redirectUrlContinue,$jobId,$postId,$skipReviewScreen,$issues,$submit)=
      $this->extractParams(['myHomeRedirectUrlContinue',
        self::$PARAM_JOB_ID,
        self::$PARAM_POST_ID,
    ...
    foreach($issues as $id=>$issue){
      ...
      $issueResponse=$api->post($command,$issueParams,$authentication,false);
  ...
  protected function doShortcodeMaintenance(array $atts){
    ...
    $attMaxIssues=$this->retrieveAttMaxIssues($atts);
    ...
    if(false) // This request is not really necessary - if the job doesn't exists, the next one (maintenanceissues) will fail
      if(!$this->retrieveJobDetails($jobId))
        throw new MyHomeException('Job not found: '.$jobId);

    // Retrieve the issues list for this job
    $command=['maintenancejobs',$jobId,'maintenanceissues'];
    $issuesResponse=$api->get($command,$authentication,true);
    ...
    //   - title: file title
    //   - url: file URL
    foreach($issuesResponse as $issueResponse){
      if(empty($issueResponse->id))
        continue;
      if(empty($issueResponse->name))
        continue;
      if(empty($issueResponse->description))
        continue;

      $issue=['id'=>$issueResponse->id,
        'title'=>$issueResponse->name,
        'description'=>$issueResponse->description,
        'deletable'=>false];

      $files=[];

      if(!empty($issueResponse->documents))
        foreach($issueResponse->documents as $document){
    ...
    $this->loadView('shortcodeMaintenanceIssues','MyHomeShortcodes',
      compact('attMaxIssues','jobId','issues','addIssue','skipReviewScreen','redirectUrl','redirectUrlError',
        'redirectUrlContinue','paramJobId','paramPostId','maxFiles'));
  }
