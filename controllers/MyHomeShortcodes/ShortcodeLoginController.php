...
class ShortcodeLoginController extends MyHomeShortcodesBaseController{
    ...
    //var_dump(PHP_URL_HOST);
		...
		$loginHelpPageId=myHome()->options->getLoginHelpPage();
		if($loginHelpPageId)
			$loginHelpUrl=get_permalink($loginHelpPageId);
		else
			$loginHelpUrl='';
		...
		$this->loadView('shortcodeLogin','MyHomeShortcodes',
		  compact('attFacebook','loginRedirect','loginHelpUrl','facebookAppId','facebookError'));
	}
    ...
    //myHome()->log->info('login() ' . serialize($params));
		...
		if($loginResponse!==null) {
          ...
          //myHome()->log->info('$authentication ' . serialize($authentication));
					...
					if($jobResponse!==null&&!empty($jobResponseArray)){
						...
						$loginError=false;
					} else myHome()->log->info('loginresponse OK but no job ' . serialize($jobResponse));
				} else myHome()->log->info('loginresponse OK but no id ' . serialize($loginResponse->id));
    }
	...
	private function checkFacebookLogin($loginRedirect){
		...
		if($user!==null){
			...
			// Partial login - the user will need to choose a job from the dropdown
				...
				$this->loadView(['shortcodeLogin','jobsList'],'MyHomeShortcodes',compact('email','picture','jobsList',
				  'loginRedirect'));
	 ...
	 */
	private function selectJob($params){
		list($email,$jobNumber)=$this->extractParams(['myHomeEmail',
		  'myHomeJobNumber'],$params);
