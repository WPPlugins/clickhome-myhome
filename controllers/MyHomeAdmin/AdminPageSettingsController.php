...
class AdminPageSettingsController extends MyHomeAdminBaseController{
  ...
  public function doGet(array $params=[]){
    ...
    $loginHelpPage=$options->getLoginHelpPage();
    ...
    $suggestedLoginHelpPage=$this->detectLoginHelpPage();

    $requirements=$this->checkRequirements();

    $this->loadView('adminPageSettings','MyHomeAdmin',
      compact('endpoint','loginPage','loginHelpPage','logoffMenuLocation','logoffOptionName','mainPage','mainLogo','bgImage','logEnabled','logLevel',
        'logMethod','contactApiKey','advertisingApiKey','suggestedLoginPage','suggestedLoginHelpPage','requirements'));
  }
  ...
  public function doPost(array $params=[]){
    list($endpoint,$loginPage,$loginHelpPage,$logoffMenuLocation,$logoffOptionName,$mainPage,$mainLogo,$bgImage,$logEnabled,$logLevel,$logMethod,
    ...
    $loginHelpPage=(int)$loginHelpPage;
...
//return;
    ...
    // If no errors are found, save the options set by the user
      ...
      $options->setLoginHelpPage($loginHelpPage);
  ...
  /**
   * Searches for a page containing the LoginHelp shortcode
   *
   * If more than one page contains this shortcode, it returns the first according to the default get_posts() order
   * (post date descending)
   *
   * @uses MyHomeShortcodes::detect()
   * @return int the login help page ID, if found, or 0 otherwise
   */
  private function detectLoginHelpPage(){
	  $suggestedLoginHelpPage=0;
	  ...
	  // Detect the Login shortcode in each page
	  foreach($pages as $page)
		  if(myHome()->shortcodes->detect($page->post_content,'LoginHelp')){
			  $suggestedLoginHelpPage=$page->ID;
			  break; // Don't look any further
		  }

	  return $suggestedLoginHelpPage;
  }
